...
let cos_integration x x' =
 ...
 | STimes(f, SVar v1::[SVar v2]) when v1=x' -> divide(SSin x,
                                               s_times(SFloat f::[SVar v2]))
 | STimes(f, SVar v1::[SVar v2]) when v2=x' -> divide(SSin x,
...
let log_integration x x' =
 ...
 | STimes(f, [SVar v]) when v=x' -> plus(times(SVar x', SLog x),
...
let rec integrate s1 s2 =
 ...
 | SPow (SPlus(STimes(c1,[SVar v]) :: [SFloat c2]), SFloat (-1.)), SVar v'
    when v'=v -> divide(SLog(SPlus( STimes(c1,[SVar v]) :: [SFloat c2] )),
                 SFloat c1)
 | SPow (SPlus (SVar x::[SFloat f]), SFloat (-1.)), SVar v
    when v = x -> SLog(SPlus (SVar x::[SFloat f]))
